...
function imgFormatter(value,row,index){
    var defaultImg = "this.src='../res/img/avatar.png'";
    return '<img id="avatarViewObj" src="'+row.avatar+'" onerror="'+defaultImg+'" style="width:128px;height:128px;">';
}
...
function getValuesFromTagbox(tagId){
    var newValueList=[];
    var tagBoxObj = $('#'+tagId);
    for(var i = 0 ; i < tagBoxObj.next().find('.textbox-value').length; i++)
        newValueList.push(tagBoxObj.next().find('.textbox-value')[i].value);
...
function appendValueToTagbox(tagId, newValue) {
    ...
    if(newValue){
        ...
        tagBoxObj.tagbox({value: newValueList});
    }
}